...
(** Represents the data of a tag in a statement. For example, in
    ...
    ["tag1"] would be a [Constant] [(tag2 : int)] would be [Type_constrained] [tag3] would
    be a [String_expression] [%here] would be a [Here_extension] *)
type t =
...
val render
  ...
  -> [ `Tag | `Tag_option | `Tag_list ] * expression
