...
```ocaml
# open Core;;
# [%log.global.debug "wow"];
Line 1, characters 1-26:
Alert ppx_log_syntax_not_in_scope: Ppx_log_syntax.Global.message
In order to use [ppx_log], you need to have a [Ppx_log_syntax] in scope. This is usually provided with [open Async], [open Async_log.Ppx_log_syntax], or [open Async_log_kernel.Ppx_log_syntax].
- : Ppx_log_syntax.return_type = `Ppx_log_syntax_implementation_missing
```
