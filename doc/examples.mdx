...
# let log = Log.For_testing.create ~map_output:Fn.id `Info in
  [%log.t log "my"];
  [%log.t.info log "world"];
my
world
- : unit = ()
# [%log.debug "should not log because debug"];
- : unit = ()
```
...
# [%log "message" (year : int) ~suffix];
(message(year 2014)(suffix CE))
- : unit = ()
# [%log "with_optional"
        ~id1:(Null : int or_null[@sexp.or_null])
        ~id2:(Some 123 : int option[@sexp.option])
        ~id3:(Some 456 : int option[@sexp.omit_nil])]
(with_optional(id2 123)(id3(456)))
- : unit = ()
```

It additionally permits optional labels instead of `[@sexp.option]`:
```ocaml
# [%log "with_optional" ?id1:(None : int option) ?id2:(Some 123 : int option)]
(with_optional(id2 123))
- : unit = ()
```
...
```ocaml
# let s = [%sexp (1234 : int)] in [%log.sexp s]
1234
- : unit = ()
# [%log.info_format "hello %s" "world"]
hello world
- : unit = ()
# let i = 3 in [%log.info_string "hello %{i#Int}"]
hello 3
- : unit = ()
# [%log "hello" (3 : int)]
(hello(3 3))
- : unit = ()
# [%log "hello" (3 : int) [@@legacy_tag_parentheses]]
(hello((3 3)))
- : unit = ()
```
...
# let verbose_log = Log.create () ~output:[ Log.Output.stderr () ] ~level:`Info ~on_error:`Raise in
  ...
  [%log.t verbose_log "hello" [@@time (Some Time_float.epoch)] [@@level Some `Error]];
  [%log.t verbose_log "hello" [@@time (Some Time_float.epoch)] [@@tags ["name", "value"]]];
...
# let `Do_not_use_because_it_will_not_log = [%log.error "this should not log"];
```
...
Use `[%log.make_raw ...]` to create a raw message, and `[%log.raw ...]` to log it. Raw
messages preserve structural information and can be serialized or passed around before
logging.

```ocaml
# let raw = [%log.make_raw "event" ~user:("alice" : string) ~code:(200 : int)] in
  [%log.raw raw];
(event(user alice)(code 200))
- : unit = ()
```

Raw messages are useful when you want to separate message creation from logging, or to
embed structured log data in error types. Use
`Ppx_log_types.{Raw_message,Message_data}.sexp_of_t_hum` to serialize a raw message:

```ocaml
# let (_raw_source_position, raw_data) = [%log.make_raw "event" ~user:("alice" : string) ~code:(200 : int)] in
  Ppx_log_types.Message_data.sexp_of_t_hum raw_data |> Core.print_s;
(event (user alice) (code 200))
- : unit = ()
```
...
let emit_warning ~(here : [%call_pos]) kind =
  [%log.info "Warning" (kind : Warning_kind.t) [@@loc here]]
;;
```

## [@list] tags
`ppx_log` allows duplicate tags, which can be useful for matching on one of many values in a
log message after it's been written.  The legacy way of doing this was by manually constructing
a `(string * string) list` and using it to populate `[@@tags]`.  Instead, you can annotate a
`_ list` type in the message with `[@list]` and one tag will be created per entry in the list.

```ocaml
# let ids = [ "abc"; "123" ] in
  [%log "one_tag_per_id" (ids : (string list[@list]))]
(one_tag_per_id(ids abc)(ids 123))
- : unit = ()
```

Empty lists produce no tags.  You can use labels to rename the tag produced.
...
# let nothing = [] in
  ...
  [%log "advanced_lists"
    (nothing : (int list[@list]))
    ~renamed_id:(ids : (string list[@list]))
    ~_:(users : (string list[@list]))]
(advanced_lists(renamed_id abc)(renamed_id 123)alice bob)
- : unit = ()
```
